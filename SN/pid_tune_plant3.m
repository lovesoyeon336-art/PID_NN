...
%% ==================== ITAE 整定固定 PID —— 对象3（Hammerstein 非线性） ====================
...
for i = 1:size(init_guesses, 1)
    cost_fn = @(x) pid_cost_plant3(x, N_tune, r_target);
...
for k = 1:N_tune
    ...
    y_k = plant_dynamics('plant3', y_1, y_2, u_k, u_1, k);
...
fprintf('===== ITAE 整定结果（对象3: Hammerstein）=====\n');
...
save(fullfile(script_dir, 'pid_tuned_params_plant3.mat'), 'Kp_opt', 'Ki_opt', 'Kd_opt');
fprintf('\n最优参数已保存至 pid_tuned_params_plant3.mat\n');

%% ==================== 绘图 ====================
fx = [1, min(500, N_tune)];
figure('Name', '整定后 PID 阶跃响应（对象3）', 'NumberTitle', 'off');
plot(1:N_tune, r_target * ones(1, N_tune), 'r', 1:N_tune, y_hist, 'b--', 'LineWidth', 1.2);
xlim(fx);
xlabel('时间步'); ylabel('输出');
legend('目标', '实际', 'Location', 'best');
title(sprintf('ITAE 整定 PID 对象3 (Kp=%.2f, Ki=%.2f, Kd=%.2f)', Kp_opt, Ki_opt, Kd_opt));
grid on;

%% ==================== 局部函数 ====================

function cost = pid_cost_plant3(x, N_sim, r_target)
    Kp = exp(x(1));
    Ki = exp(x(2));
    Kd = exp(x(3));
