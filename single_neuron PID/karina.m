...
if M==1
    wkp(k)=wkp_1+xitep*x(1)*u_1;
    wki(k)=wki_1+xitei*x(2)*u_1;
    wkd(k)=wkd_1+xited*x(3)*u_1;
    K=0.06;
elseif M==2
    wkp(k)=wkp_1+xitep*error(k)*x(1);%这里用的是delta公式，效果还不如用u1
    wki(k)=wki_1+xitei*error(k)*x(2);
    wkd(k)=wkd_1+xited*error(k)*x(3);
    K=0.12;
elseif M==3
